namespace Core.Models
{
        ...
        public override double Evaluate(ModelManager modelManager)
        {
            ...
            if (!param.IsScalar)
            {
                throw new InvalidOperationException(
                    $"Parameter '{ParameterName}' is indexed, cannot evaluate as scalar");
            }
    ...
    /// <summary>
    /// Represents a reference to a decision expression
    /// </summary>
    public class DecisionExpressionExpression : Expression
    {
        public string Name { get; }
        public int? Index { get; }
        public Expression? IndexExpression { get; } // For dynamic indices like 'i'

        // For scalar dexpr or indexed with literal index
        public DecisionExpressionExpression(string name, int? index = null)
        {
            Name = name;
            Index = index;
        }

        // For indexed dexpr with expression-based index
        public DecisionExpressionExpression(string name, Expression indexExpr)
        {
            Name = name;
        ...
        public override double Evaluate(ModelManager modelManager)
        {
            if (!modelManager.DecisionExpressions.TryGetValue(Name, out var dexpr))
            {
                throw new InvalidOperationException($"Decision expression '{Name}' not found");
            }
            ...
            if (IndexExpression != null)
            {
                ...
                actualIndex = (int)Math.Round(IndexExpression.Evaluate(modelManager));
            }

            // Evaluate the dexpr
            if (actualIndex.HasValue)
            {
                return dexpr.Evaluate(modelManager, actualIndex.Value);
            }
            else
            {
                return dexpr.Evaluate(modelManager);
            }
        }
        ...
        public override Expression Simplify(ModelManager? modelManager = null)
        {
            if (modelManager != null)
            {
                // Could potentially inline the dexpr expression here
                try
                {
                    if (!modelManager.DecisionExpressions.TryGetValue(Name, out var dexpr))
                        return this;

                    // For scalar dexpr, we could inline it
                    if (!dexpr.IsIndexed && Index == null && IndexExpression == null)
                    {
                        // Return the underlying expression (inlining)
                        return dexpr.Expression.Simplify(modelManager);
                    }
                }
                catch
                {
                    // If simplification fails, return as-is
                }
            }
